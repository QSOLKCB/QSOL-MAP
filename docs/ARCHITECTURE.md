...
The reported candidate list is also an exact deterministic cutoff: descending positive delta, then ascending frame index. When more than 16 candidates exist, an omitted candidate may equal the weakest reported delta only at a later frame; an earlier equal-delta frame would displace the reported cutoff item. If every transition is a candidate, the exact omitted positive-delta mass must fit those cutoff allowances and the summary maximum equals the strongest reported candidate. If a declared maximum exceeds the strongest reported candidate, it cannot belong to an omitted stronger candidate and therefore must be realizable by an actual non-candidate transition, with enough positive mass outside the reported set to attain that maximum.
