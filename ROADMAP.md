...
- [x] Bind complete two-sample channels to signed endpoint-compatible PCM16 witnesses, exact long energy and one joint Gram assignment, not merely `D^2 + N^2 = 2*windowed_energy`.
...
- [x] Derive **per-bin coefficient divisors** from the frozen integer butterfly schedule and enforce them on every event's reported components and every aggregate power, including multi-event sums. Enforce the derived event denominator/numerator gcd divisors, including the `2^64` denominator divisor.
...
- [x] Enforce endpoint Cauchy bounds and exact constant/alternating-window PCM16 witnesses when equality holds.
...
- [x] Require one joint nonnegative integer residual-power allocation matching event row totals, aggregate column totals, selected-bin exactness and tie-aware capacities.
...
- [x] Require one joint endpoint-compatible PCM16 assignment for three- and four-frame multichannel Gram data and long energies, not just realizable diagonals or separate pairwise witnesses; specification section 8.2 defines the bounded solver.
...
- [x] Bind one-frame multichannel relationship dot products to the signed PCM samples recovered from reported DC coefficients, not just their magnitudes.
- [x] Account for omitted transient candidates and transition multiplicity in compact summary verification.
- [x] Enforce the deterministic top-16 transient cutoff, including ascending-frame tie ordering and known-neighbor feasibility in mixed and all-candidate sets.
- [x] Require a stronger-than-reported transient maximum to fit an actual non-candidate transition, known neighboring energies, onset-threshold bounds and unreported mass.
- [x] Cap total unreported mass using non-candidate capacities and exactly the declared number of omitted-candidate replacements.
- [x] Couple the exact all-zero long-energy state to the short profile: if every covering long-event energy is zero, the channel's transient candidate count/list/sum/maximum must all be zero.
...
The full normative long transform is in specification section 4.1. Its **per-bin coefficient divisors** are derived by propagating integer divisibility through each butterfly: multiplication scales a divisor by the absolute Q15 factor and sums/differences take the gcd. Every event's reported components obey `D_real[k]` and `D_imag[k]`; every aggregate bin, including multi-event sums, obeys `g[k]=gcd(D_real[k]^2,D_imag[k]^2)`. Each event denominator obeys `gcd(g[k])=2^64`; each numerator obeys `gcd(k*g[k])`. Divisibility survives summation. Single-event perfect-square and two-square restrictions do not survive summation and are not imposed on multi-event aggregates. Section 4.2 makes this distinction normative.

The small-window checks reject unattainable one/two-sample energies without claiming complete compact-only feasibility for arbitrary lengths. Complete three- and four-frame multichannel checks are exact for their joint Gram, weighted-energy and endpoint power/sign evidence. Section 8.2 derives `A=(D+N)/2=x+3z`, `B=(D-N)/4=y+2t`, and `(x-3z)^2=2*(W-4y^2-16t^2)-A^2`. For three frames `t=0`; four frames enumerate bounded signed PCM16 `t`, then recover the remaining coordinates by exact square/divisibility checks. One shared assignment must satisfy all channel dot products. The solver does not certify unreported spectrum or source commitments.

The single-event two-square filter remains a bounded necessary check, not general large-integer factorization. Writer-side PCM validation binds the actual sample payload; it cannot recompute the original RIFF container hash from a `PCM16Wave` object that does not retain those bytes. Write-completion checks prevent false success receipts, but do not promise rollback of partial output or durable storage.

Compact conformance preserves signed endpoint evidence. Reported one-event DC/Nyquist signs must satisfy the even/odd long-window congruences, while omitted endpoints may use either sign. Endpoint Cauchy equality forces a constant or alternating-constant windowed vector that must divide by every window weight into PCM16 samples and reproduce the same energy and both endpoints. Two-sample witnesses derive `x=(D+N)/2`, `y=(D-N)/4` exactly and use those same signed samples for energy and Gram data. For a one-frame multichannel source, signed DC coefficients fix the sole samples and every relationship dot product equals their signed product.

Per-event top-K bounds preserve descending-power/ascending-bin ties; an earlier omitted bin cannot tie the weakest selected bin. Per-event centroid numerators are bounded below by selected weighted power and above by assigning omitted power to bin 512. In addition, row residuals (event denominator minus selected powers) and column residuals (aggregate bin minus selected subtotals) must admit one integer max-flow allocation, forbidding residual power in selected cells and respecting each event's tie-aware cutoff. This does not prove that a complete transform or every centroid moment has a common waveform witness.

Transient conformance treats the reported 16 strongest candidates as a deterministic cutoff. Known neighboring energies constrain eligible omitted candidates in mixed and all-candidate sets. Previous energy `P` requires `d>=ceil(P/2)`, current energy `C` requires `d>=C-floor(2C/3)`, and two known energies require exact `d=C-P` and the candidate predicate. A stronger-than-reported maximum must be attainable by a non-candidate transition honoring known neighbors and source-sized threshold bounds, with enough unreported mass. Total mass also fits the sum of non-candidate caps after exactly the declared number of feasible omitted-candidate cap replacements. These are necessary compact conditions, not a reconstruction of the full short-energy chain. All-zero covering long energies still force all-zero/empty transients.

The three-frame mono check in specification section 8.1 is exact for its weighted energy and two endpoint powers/signs. It does not extend to three-sample tails of longer sources or prove the remaining spectral/source commitments. Regression coverage includes the rehashed energy-2 example, 14625 exhaustive small energy/endpoint cases, signed PCM16 limits and unchanged sidecar round trips.
